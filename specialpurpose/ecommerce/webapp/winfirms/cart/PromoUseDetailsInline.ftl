...
<div class="cart_totals">
	<div class="col-md-12 push-md-12 no-padding">
    <h4 class="uppercase">${uiLabelMap.OrderPromotionInformation}:</h4>
    <table class="table table-bordered col-md-12">
    	<tbody>
    		<tr>
				<th>${uiLabelMap.OrderPromotionsApplied}:</th>
				<td>
                ...
                <span class="amount">
                    ...
                    <#if (productPromoUseInfo.totalDiscountAmount != 0)> - ${uiLabelMap.PromoTotalValue} <@ofbizCurrency amount=(-1*productPromoUseInfo.totalDiscountAmount) isoCode=shoppingCart.getCurrency()/></#if>
                    <#if productPromoUseInfo.productPromoCodeId?has_content>
                        <a href="<@ofbizUrl>removePromotion?promoCode=${productPromoUseInfo.productPromoCodeId!}</@ofbizUrl>" >${uiLabelMap.CommonDelete}</a>
                    </#if>
                </span>
                <#if (productPromoUseInfo.quantityLeftInActions > 0)>
                    <span class="amount">- Could be used for ${productPromoUseInfo.quantityLeftInActions} more discounted item<#if (productPromoUseInfo.quantityLeftInActions > 1)>s</#if> if added to your cart.</span>
                </#if>
            </#list>
				</td>
			</tr>
			<tr>
				<th>${uiLabelMap.OrderCartItemUseinPromotions}:</th>
				<td>
			                ...
			                <li>${uiLabelMap.OrderItemN} ${cartLineIndex+1} [${cartLine.getProductId()!}] - ${cartLine.getPromoQuantityUsed()?string.number}/${cartLine.getQuantity()?string.number} ${uiLabelMap.CommonUsed} - ${cartLine.getPromoQuantityAvailable()?string.number} ${uiLabelMap.CommonAvailable}
			                    <ul>
			                        <#list cartLine.getQuantityUsedPerPromoFailedIter() as quantityUsedPerPromoFailedEntry>
			                            ...
			                            <li>&nbsp;&nbsp;-&nbsp;${uiLabelMap.CommonCouldBeUsedAs} <#if isQualifier>${uiLabelMap.CommonQualifier}<#else>${uiLabelMap.CommonBenefit}</#if> ${uiLabelMap.OrderOfPromotion}</li>
