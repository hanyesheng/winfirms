...
<div id="div3">
	<script type="text/javascript">
	        ...
	        var str = info.split('~');
	        var checkUrl = '/store/control/investorsCategoryAjaxFired';
	        if(checkUrl.search("http"))
	            var ajaxUrl = '/store/control/investorsCategoryAjaxFired';
	        else
	            var ajaxUrl = '/store/control/investorsCategoryAjaxFiredSecure';   
	            $("#loadgif").show(); 
		...
		<div class="row">
			<div class="col-md-12 space30 investorsProduct">
				<table class="table table-hover">
					<thead>
						<tr>
							<th colspan="2" class="text-center investorsName">${uiLabelMap.Investors}</th>
							<th>${uiLabelMap.BriefIntroduction}</th>
							<th>${uiLabelMap.StagePrefer}</th>
							<th>${uiLabelMap.FieldPrefer}</th>
						</tr>
					</thead>
					<tbody class="investorsProduct">
						<#if productCategoryMembers?has_content>
					        ...
					        ${screens.render(investorsProductsummaryScreen)}
					        </#list>
						<#else>
						    <hr />
