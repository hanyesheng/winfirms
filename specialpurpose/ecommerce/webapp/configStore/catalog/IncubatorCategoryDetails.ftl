...
<div id="div3">
	<script type="text/javascript">
	        ...
	        var str = info.split('~');
	        var checkUrl = '/store/control/incubatorCategoryAjaxFired';
	        if(checkUrl.search("http"))
	            var ajaxUrl = '/store/control/incubatorCategoryAjaxFired';
	        else
	            var ajaxUrl = '/store/control/incubatorCategoryAjaxFiredSecure';   
	            $("#loadgif").show(); 
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-12 space30 incubatorProduct">
				<table class="table table-hover">
					<thead>
						<tr>
							<th colspan="2" class="text-center investorsName">${uiLabelMap.Invubator}</th>
							<th>${uiLabelMap.BriefIntroduction}</th>
							<th>${uiLabelMap.EcommerceLocation}</th>
							<th>${uiLabelMap.CommonTag}</th>
						</tr>
					</thead>
					<tbody class="incubatorProduct">
						<#if productCategoryMembers?has_content>
						    <#list productCategoryMembers as productCategoryMember>
					        ${setRequestAttribute("optProductId", productCategoryMember.productId)}
					        ${setRequestAttribute("productCategoryMember", productCategoryMember)}
					        ${setRequestAttribute("listIndex", productCategoryMember_index)}
					        ${screens.render(incubatorProductsummaryScreen)}
					        </#list>
						<#else>
						    <hr />
