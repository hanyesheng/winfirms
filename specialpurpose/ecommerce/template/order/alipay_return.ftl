...
<!--20170110clgalipay23-->
                ...
                <#assign return_url = request.getAttribute("return_url")?if_exists/>
                	${uiLabelMap.Successpayall}<br>
				${uiLabelMap.Successpay1}:${get_order?if_exists}  <br>
				${uiLabelMap.Successpay2}:${get_total_fee?if_exists} <br>
				${uiLabelMap.Successpay3}:${get_subject?if_exists} <br>
				${uiLabelMap.Successpay4}:${get_body?if_exists}  <br>
				${uiLabelMap.Successpay5}:${result?if_exists}[${trade_status?if_exists}]<br>
				<#--${responseTxt?if_exists}-->
				<br><a href="${return_url?if_exists}">${uiLabelMap.Successpay6}</a>
				<#elseif flag=="fail">
