...
public class AlipayEvents {
    ...
    public static String callAliPay(HttpServletRequest request, HttpServletResponse response) {
    	...
    	//下面的是改成远程的代码
    	/*String path = "http://winfirms.net";
    	String return_url = "https://winfirms.net:8443" + UtilProperties.getPropertyValue(configString, "alipay.partner.return_url");
    	String notify_url = path + UtilProperties.getPropertyValue(configString, "alipay.partner.notify_url");*/
    	...
    	//List orderAdjustments = delegator.findByAnd("OrderAdjustment", UtilMisc.toMap("orderId",orderId,"orderAdjustmentTypeId","SHIPPING_CHARGES")); //
    		List orderAdjustments = delegator.findByAnd("OrderAdjustment", UtilMisc.toMap("orderId",orderId,"orderAdjustmentTypeId","SHIPPING_CHARGES"),new ArrayList() , true);
    	...
    	//GenericValue orderItemShipGroup = delegator.findByPrimaryKey("OrderItemShipGroup", UtilMisc.toMap("orderId",orderId,"shipGroupSeqId","00001")); //
    		GenericValue orderItemShipGroup = delegator.findOne("OrderItemShipGroup", UtilMisc.toMap("orderId",orderId,"shipGroupSeqId","00001"), true);
    	...
    	String price = orderTotal.replaceAll(",","");
    	...
    	try {
    		...
    		List orderItems = delegator.findByAnd("OrderItem", UtilMisc.toMap("orderId",orderId), new ArrayList(), true);
        ...
        if(i > 1)       
        	...
        	{ quantity = UtilFormatOut.formatQuantity(counter);
        	  ...
        	  String formatPrice = UtilFormatOut.formatPrice(Double.valueOf(total/counter));//The average unit price of goods
        	  price = formatPrice.replaceAll(",","");
        	}
