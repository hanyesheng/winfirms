 ...
 */
package org.apache.ofbiz.accounting.thirdparty.alipay;
...
import org.apache.ofbiz.accounting.thirdparty.alipay.util.CheckURL;
...
public class AlipayServices {
	...
	public static String aliPayNotify(HttpServletRequest request, HttpServletResponse response) {
		...
		String partner = UtilProperties.getPropertyValue("alipay.properties","alipay.partner"); // partner partner id (must be filled out)
		String privateKey = UtilProperties.getPropertyValue("alipay.properties", "alipay.partner.key"); // partner check the
															// corresponding
															// transaction
															// security code
															// (required)
		...
		String alipayNotifyURLForHttp = UtilProperties.getPropertyValue("alipay.properties", "alipay.NotifyURLForHttp") + "&partner=" + partner + "&notify_id=" + request.getParameter("notify_id");
		...
		for (Iterator iter = requestParams.keySet().iterator(); iter.hasNext();) {
			...
			Debug.logInfo("*******NAME***"+name,module);
			Debug.logInfo("*******Value***"+valueStr,module);
		...
		Debug.logInfo("*************"+request.getParameter("trade_status"),module);
	if (mysign.equals(request.getParameter("sign")) && responseTxt.equals("true")) {
			if (request.getParameter("trade_status").equals("WAIT_BUYER_PAY")) {
			...
			} else if (request.getParameter("trade_status").equals("TRADE_SUCCESS")) {
				...
				/*
				 * //Accept payment,Pay for the right to take over after the orders automatically become approved by
				 * 
				 * // get the order header & payment preferences GenericValue
				 * orderHeader = null; try { orderHeader = delegator.findByPrimaryKey("OrderHeader", UtilMisc.toMap("orderId", orderId)); } catch
				 * (GenericEntityException e) { Debug.logError(e,
				 * "Problems reading order header from datasource.", module);
 				 * }
				 * 
				 * Double grandTotal = new Double(0.00); if (orderHeader != null) { grandTotal = orderHeader.getDouble("grandTotal"); }
				 * 
				 * // get the payment types to receive List paymentMethodTypes = null;
				 * 
				 * try { List pmtFields = UtilMisc.toList(new EntityExpr("paymentMethodTypeId", EntityOperator.NOT_EQUAL, "EXT_OFFLINE")); paymentMethodTypes =
				 * delegator.findByAnd("PaymentMethodType", pmtFields); } catch (GenericEntityException e) { Debug.logError(e, "Problems getting payment types", module);}
				 * 
				 ...
				 * placingCustomer = null; try { List pRoles = delegator.findByAnd("OrderRole", UtilMisc.toMap("orderId", orderId, "roleTypeId", "PLACING_CUSTOMER")); if (pRoles !=
				 * null && pRoles.size() > 0) placingCustomer = EntityUtil.getFirst(pRoles); } catch (GenericEntityException e) { Debug.logError(e, "Problems looking up order payment preferences", module);
				...
				try {
				 ...
				 try {
						GenericValue orderPaymentPreference = EntityQuery.use(delegator).from("OrderPaymentPreference").where("orderId", orderId, "paymentMethodTypeId", "EXT_ALIPAY").queryFirst();
					 ...
					 try {
							GenericValue orderPaymentPreference = EntityQuery.use(delegator).from("OrderPaymentPreference").where("orderId", orderId, "paymentMethodTypeId", "COMPANY_ACCOUNT").queryFirst();
							 if (orderPaymentPreference != null) {
			                        orderPaymentPreference.set("statusId", "PAYMENT_SETTLED");
			                        orderPaymentPreference.store();						 
							 }
